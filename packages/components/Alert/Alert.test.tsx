...
import { SgAlert } from "./index";

import Alert from "./Alert.vue";
import SgIcon from "../Icon/Icon.vue";
...
describe("Alert.vue", () => {
  ...
  it("should render the alert with default props", () => {
    ...
    // close icon
    const iconElement = wrapper.findComponent(SgIcon);
  ...
  it("should emit close event when close icon is clicked", () => {
    ...
    wrapper.findComponent(SgIcon).trigger("click");
  ...
  it("should support centering text", () => {
    ...
    //class
    const rootNode = wrapper.find(".sg-alert");
    expect(rootNode.classes()).toContain("is-center");
  });
  ...
  it("should not render close icon when closable is false", () => {
    ...
    expect(wrapper.find(".sg-alert__close").exists()).toBe(false);
  });
  ...
  it("should toggle visibility when open and close methods are called", async () => {
    ...
    expect(wrapper.find(".sg-alert").attributes().style).toBe("display: none;");
    await wrapper.vm.open();
    expect(wrapper.find(".sg-alert").attributes().style).toBe("");
  });
  test('close', () => {
    const wrapper = mount(() => <Alert closeText={'close'} />)
    expect(wrapper.find(".sg-alert__close").text()).toBe('close');
  })
});

describe("Alert/index", () => {
  it("should be exported with withInstall()", () => {
    expect(SgAlert.install).toBeDefined();
  });
  it("component should be exported", () => {
    expect(SgAlert).toBe(Alert);
  });

  // 可选
  it("should enhance Alert component", () => {
    const enhancedAlert = withInstall(Alert);
    expect(enhancedAlert).toBe(SgAlert);
  });
